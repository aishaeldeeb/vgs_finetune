...
class LitAVAligner(pl.LightningModule, PyTorchModelHubMixin, repo_url="https://github.com/mhamilton723/DenseAV", license="mit", tags=["denseav"]):
    ...
    def _calc_recalls(self, sim):
        top_10_a = sim.topk(2, 0).indices == torch.arange(sim.shape[0]).unsqueeze(0)
        top_10_i = (sim.topk(2, 1).indices == torch.arange(sim.shape[0]).unsqueeze(1)).permute(1, 0)
...
def my_app(cfg: DictConfig) -> None:
    ...
    elif cfg.dataset_name in {"vistext"}:
        val_args = dict(check_val_every_n_epoch=5)
    else:
        val_args = dict(val_check_interval=1)
