...
class AVDataset(ABC, Dataset):
    ...
    def _get_audio_file(self, row) -> str:
        return self._audio_root() + "/" + row["id"] + ".mov"
    ...
    def _filter_valid_metadata(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.use_frames:
            missing_frame_files = set(item for sublist in df['frame_files'].tolist() for item in sublist) - self.all_frame_files
        ...
        if (percent_missing <= self._missing_threshold()):
            print(f"Too many missing files: %{round(percent_missing * 100.0, 2)}")
...
class VisTexDataset(AVDataset):
    
    def _load_info(self, split) -> pd.DataFrame:
        # Read JSON, ensuring 'caption_id' is treated as a string
        df = pd.read_json(join(self.root, f"data_{split}.json"), dtype={"caption_id": str})

        # Add 'id' column as a copy of 'caption_id'
        df["id"] = df["caption_id"]
        

        # Duplicate the df: the first half will have L1 captions, the second half will have L2 captions
        df_l1 = df.copy(deep=True)
        df_l23 = df.copy(deep=True)

        # Check the format
        # print(df["caption_id"].head())
        # print(df_l1["caption_id"].head())

        if self.use_caption:
            # Assign the appropriate captions to each copy
            df_l1["text"] = df_l1["caption_L1"]  # For L1 captions
            df_l23["text"] = df_l23["caption_L2L3"]  # For L2L3 captions

        # Add a suffix to the 'id' column to differentiate between L1 and L2L3
        df_l1["id"] = df_l1["id"].apply(lambda x: f"{x}_l1") # Append "_l1" to L1 entries
        df_l23["id"] = df_l23["id"].apply(lambda x: f"{x}_l23")  # Append "_l23" to L2 entries

        # Concatenate the two DataFrames (L1 and L2L3 captions) back together
        df = pd.concat([df_l1, df_l23], ignore_index=True)

        #constrct frame_files (train_1_01_l23.png)
        if split == "train":
            df["image_filename"] = df["id"].apply(lambda x: f"{split}_{x}.png")
        elif split == "val":
            df["image_filename"] = df["id"].apply(lambda x: f"validation_{x}.png")
        elif split == "test":
            df["image_filename"] = df["id"].apply(lambda x: f"tst_{x}.png")
        
        # Construct audio_filename (train_1_01_L23.wav)
        df["audio_filename"] = df["image_filename"].str.replace(".png", ".wav")
        
        print(f"Number of files processed during the {split} stage", df.shape[0])
        return df
    ...
    def _get_frame_files(self, row) -> List[str]:
        # extract the frames paths
        return [join(self._frame_root(), row["image_filename"])]

    def _get_audio_file(self, row) -> str:
        # extract the audio paths
        return join(self._audio_root(), row["audio_filename"])
    ...
    def _expected_num_frames(self): #not sure what this means
        return 1
...
class AVDataModule(pl.LightningDataModule):
    ...
    def get_dataset_by_name(self, name, stage, data_for_plotting, n_frames=None):
        ...
        if self.audio_model_type == "hubert":
            ...
            audio_transform = None
        #TBR: WHAT SAMPLE RATE SHOULD I USE?
        ...
        #Setting up dataset paramswhat is this?
        data_args = dict(
        ...
        train_args = dict(
            ...
            extra_audio_masking=self.extra_audio_masking, #TBR: what is audio masking?
            neg_audio=self.neg_audio,
            quad_mixup=self.quad_mixup,#TBR
            bg_mixup=self.bg_mixup,#TBR
            patch_mixup=self.patch_mixup,#TBR
            patch_size=self.patch_size,#TBR
            audio_aug=train_audio_aug#TBR
        )
        val_args = dict(
        ...
        if name == "vistext":
             dataset_constructor = VisTexDataset
